...
async function scanBlock(height) {
  ...
  console.log(`Block ${height} scanned | found ${found}`);
}
...
async function main() {
  ...
  console.log(`Live indexer starting at ${height}`);
  ...
  while (true) {
    try {
      ...
      if (height <= tip) {
        console.log(`Syncing from ${height} to tip ${tip}`);
        ...
        console.log(`Caught up to tip ${tip}. Waiting for new blocks...`);
      } else {
        console.log(`At tip. Last scanned ${height - 1}. Waiting...`);
      }
